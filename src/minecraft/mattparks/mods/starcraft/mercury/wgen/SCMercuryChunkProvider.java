...
import mattparks.mods.starcraft.mercury.wgen.dungeon.SCMercuryRoomBoss;
import mattparks.mods.starcraft.mercury.wgen.dungeon.SCMercuryRoomChests;
import mattparks.mods.starcraft.mercury.wgen.dungeon.SCMercuryRoomEmpty;
import mattparks.mods.starcraft.mercury.wgen.dungeon.SCMercuryRoomSpawner;
import mattparks.mods.starcraft.mercury.wgen.dungeon.SCMercuryRoomTreasure;
...
public class SCMercuryChunkProvider extends ChunkProviderGenerate
{
    ...
    public SCMercuryBiomeDecorator biomedecoratorplanet = new SCMercuryBiomeDecorator();
    ...
    {
        this.dungeonGenerator.otherRooms.add(new SCMercuryRoomEmpty(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.otherRooms.add(new SCMercuryRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.otherRooms.add(new SCMercuryRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.otherRooms.add(new SCMercuryRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.otherRooms.add(new SCMercuryRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.otherRooms.add(new SCMercuryRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        ...
        this.dungeonGenerator.otherRooms.add(new SCMercuryRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.otherRooms.add(new SCMercuryRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.otherRooms.add(new SCMercuryRoomChests(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.otherRooms.add(new SCMercuryRoomChests(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.bossRooms.add(new SCMercuryRoomBoss(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.treasureRooms.add(new SCMercuryRoomTreasure(null, 0, 0, 0, ForgeDirection.UNKNOWN));
    }

    private BiomeGenBase[] biomesForGeneration = { SCMercuryBiomeGenBase.mercuryFlat };

    private final GCCoreMapGenBaseMeta caveGenerator = new SCMercuryGenCaves();
    ...
    public SCMercuryChunkProvider(World par1World, long par2, boolean par4)
    {
    ...
    public void createCraters(int chunkX, int chunkZ, short[] chunkArray, byte[] metaArray)
    {
        for (int cx = chunkX - 2; cx <= chunkX + 2; cx++)
        {
            ...
            {
                for (int x = 0; x < SCMercuryChunkProvider.CHUNK_SIZE_X; x++)
                {
                    for (int z = 0; z < SCMercuryChunkProvider.CHUNK_SIZE_Z; z++)
                    {
                        if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getNoise(x * SCMercuryChunkProvider.CHUNK_SIZE_X + x, cz * SCMercuryChunkProvider.CHUNK_SIZE_Z + z) / SCMercuryChunkProvider.CRATER_PROB)
                        {
    ...
    public void generateTerrain(int chunkX, int chunkZ, short[] idArray, byte[] metaArray)
    {
        ...
        for (int x = 0; x < SCMercuryChunkProvider.CHUNK_SIZE_X; x++)
        {
            for (int z = 0; z < SCMercuryChunkProvider.CHUNK_SIZE_Z; z++)
            {
                ...
                for (int y = 0; y < SCMercuryChunkProvider.CHUNK_SIZE_Y; y++)
                {
                    if (y < SCMercuryChunkProvider.MID_HEIGHT + yDev)
                    {
    ...
    {
        return SCMercuryConfigManager.generateOtherMods ? "RandomLevelSource" : "MercuryLevelSource";
    }
    ...
    public Chunk provideChunk(int par1, int par2)
    {
        ...
        final Chunk var4 = new Chunk(this.worldObj, ids, meta, par1, par2);
         ...
         SCMercuryConfigManager.generateOtherMods)
         {
