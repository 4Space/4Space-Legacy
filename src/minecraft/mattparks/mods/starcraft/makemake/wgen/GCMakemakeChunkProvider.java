...
public class GCMakemakeChunkProvider extends ChunkProviderGenerate
{
    final short topBlockID = (short) GCMakemakeBlocks.MakemakeGrass.blockID;
    final byte topBlockMeta = 5;
    final short fillBlockID = (short) GCMakemakeBlocks.MakemakeDirt.blockID;
    final byte fillBlockMeta = 3;
    final short lowerBlockID = (short) GCMakemakeBlocks.MakemakeStone.blockID;
    ...
    public GCMakemakeBiomeDecorator biomedecoratorplanet = new GCMakemakeBiomeDecorator(GCMakemakeBiomeGenBase.makemakeFlat);

    private final World worldObj;
    {
    }

    private BiomeGenBase[] biomesForGeneration = { GCMakemakeBiomeGenBase.makemakeFlat };
    ...
    public GCMakemakeChunkProvider(World par1World, long par2, boolean par4)
    {
    ...
    public void generateTerrain(int chunkX, int chunkZ, short[] idArray, byte[] metaArray)
    {
        ...
        for (int x = 0; x < GCMakemakeChunkProvider.CHUNK_SIZE_X; x++)
        {
            for (int z = 0; z < GCMakemakeChunkProvider.CHUNK_SIZE_Z; z++)
            {
                ...
                for (int y = 0; y < GCMakemakeChunkProvider.CHUNK_SIZE_Y; y++)
                {
                    if (y < GCMakemakeChunkProvider.MID_HEIGHT + yDev)
                    {
    ...
    {
        return GCMakemakeConfigManager.generateOtherMods ? "RandomLevelSource" : "VenusLevelSource";
    }
    ...
    public Chunk provideChunk(int par1, int par2)
    {
        ...
        final Chunk var4 = new Chunk(this.worldObj, ids, meta, par1, par2);
         ...
         GCMakemakeConfigManager.generateOtherMods)
         {
