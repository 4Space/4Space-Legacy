...
import mattparks.mods.starcraft.pluto.wgen.dungeon.GCPlutoRoomBoss;
import mattparks.mods.starcraft.pluto.wgen.dungeon.GCPlutoRoomChests;
import mattparks.mods.starcraft.pluto.wgen.dungeon.GCPlutoRoomEmpty;
import mattparks.mods.starcraft.pluto.wgen.dungeon.GCPlutoRoomSpawner;
import mattparks.mods.starcraft.pluto.wgen.dungeon.GCPlutoRoomTreasure;
...
public class GCPlutoChunkProvider extends ChunkProviderGenerate
{
    ...
    public Chunk provideChunk(int par1, int par2)
    {
        ...
        final Chunk var4 = new Chunk(this.worldObj, ids, meta, par1, par2);
         ...
         GCPlutoConfigManager.generateOtherMods)
         {
        ...
        for (int cx = chunkX - 2; cx <= chunkX + 2; cx++)
        {
            for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++)
            {
                for (int x = 0; x < GCPlutoChunkProvider.CHUNK_SIZE_X; x++)
                {
                    for (int z = 0; z < GCPlutoChunkProvider.CHUNK_SIZE_Z; z++)
                    {
                        if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getNoise(x * GCPlutoChunkProvider.CHUNK_SIZE_X + x, cz * GCPlutoChunkProvider.CHUNK_SIZE_Z + z) / GCPlutoChunkProvider.CRATER_PROB)
                        {
    ...
    {
        return GCPlutoConfigManager.generateOtherMods ? "RandomLevelSource" : "VenusLevelSource";
    }
