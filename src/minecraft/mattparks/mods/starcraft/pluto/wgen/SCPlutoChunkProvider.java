...
import mattparks.mods.starcraft.pluto.wgen.dungeon.SCPlutoRoomBoss;
import mattparks.mods.starcraft.pluto.wgen.dungeon.SCPlutoRoomChests;
import mattparks.mods.starcraft.pluto.wgen.dungeon.SCPlutoRoomEmpty;
import mattparks.mods.starcraft.pluto.wgen.dungeon.SCPlutoRoomSpawner;
import mattparks.mods.starcraft.pluto.wgen.dungeon.SCPlutoRoomTreasure;
...
public class SCPlutoChunkProvider extends ChunkProviderGenerate
{
    final short topBlockID = (short) PlutoBlocks.PlutoGrass.blockID;
    final byte topBlockMeta = 5;
    final short fillBlockID = (short) PlutoBlocks.PlutoDirt.blockID;
    final byte fillBlockMeta = 3;
    final short lowerBlockID = (short) PlutoBlocks.PlutoStone.blockID;
    ...
    public SCPlutoBiomeDecorator biomedecoratorplanet = new SCPlutoBiomeDecorator(SCPlutoBiomeGenBase.plutoFlat);

    private final World worldObj;
    private final GCCoreMapGenDungeon dungeonGenerator = new GCCoreMapGenDungeon(PlutoBlocks.PlutoBrick.blockID, 14, 8, 16, 3);

    {
        this.dungeonGenerator.otherRooms.add(new SCPlutoRoomEmpty(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.otherRooms.add(new SCPlutoRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.otherRooms.add(new SCPlutoRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.otherRooms.add(new SCPlutoRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.otherRooms.add(new SCPlutoRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.otherRooms.add(new SCPlutoRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        ...
        this.dungeonGenerator.otherRooms.add(new SCPlutoRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.otherRooms.add(new SCPlutoRoomSpawner(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.otherRooms.add(new SCPlutoRoomChests(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.otherRooms.add(new SCPlutoRoomChests(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.bossRooms.add(new SCPlutoRoomBoss(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.treasureRooms.add(new SCPlutoRoomTreasure(null, 0, 0, 0, ForgeDirection.UNKNOWN));
    }

    private BiomeGenBase[] biomesForGeneration = { SCPlutoBiomeGenBase.plutoFlat };

    private final GCCoreMapGenBaseMeta caveGenerator = new SCPlutoGenCaves();
    ...
    public SCPlutoChunkProvider(World par1World, long par2, boolean par4)
    {
    ...
    public void createCraters(int chunkX, int chunkZ, short[] chunkArray, byte[] metaArray)
    {
        for (int cx = chunkX - 2; cx <= chunkX + 2; cx++)
        {
            ...
            {
                for (int x = 0; x < SCPlutoChunkProvider.CHUNK_SIZE_X; x++)
                {
                    for (int z = 0; z < SCPlutoChunkProvider.CHUNK_SIZE_Z; z++)
                    {
                        if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getNoise(x * SCPlutoChunkProvider.CHUNK_SIZE_X + x, cz * SCPlutoChunkProvider.CHUNK_SIZE_Z + z) / SCPlutoChunkProvider.CRATER_PROB)
                        {
    ...
    public void generateTerrain(int chunkX, int chunkZ, short[] idArray, byte[] metaArray)
    {
        ...
        for (int x = 0; x < SCPlutoChunkProvider.CHUNK_SIZE_X; x++)
        {
            for (int z = 0; z < SCPlutoChunkProvider.CHUNK_SIZE_Z; z++)
            {
                ...
                for (int y = 0; y < SCPlutoChunkProvider.CHUNK_SIZE_Y; y++)
                {
                    if (y < SCPlutoChunkProvider.MID_HEIGHT + yDev)
                    {
    ...
    {
        return SCPlutoConfigManager.generateOtherMods ? "RandomLevelSource" : "VenusLevelSource";
    }
    ...
    public Chunk provideChunk(int par1, int par2)
    {
        ...
        final Chunk var4 = new Chunk(this.worldObj, ids, meta, par1, par2);
         ...
         SCPlutoConfigManager.generateOtherMods)
         {
