package mattparks.mods.starcraft.uranus.wgen;
...
import mattparks.mods.starcraft.uranus.SCUranusConfigManager;
...
public class SCUranusChunkProvider extends ChunkProviderGenerate
{
    final short topBlockID = (short) JupiterBlocks.UranusNitrogen.blockID;
    final byte topBlockMeta = 5;
    final short fillBlockID = (short) JupiterBlocks.UranusHelium.blockID;
    final byte fillBlockMeta = 3;
    final short lowerBlockID = (short) JupiterBlocks.UranusHydrogen.blockID;
    ...
    public SCUranusBiomeDecorator biomedecoratorplanet = new SCUranusBiomeDecorator(SCUranusBiomeGenBase.uranusFlat);

    private final World worldObj;
    private final GCCoreMapGenDungeon dungeonGenerator = new GCCoreMapGenDungeon(JupiterBlocks.UranusHydrogen.blockID, 14, 8, 16, 3);
    //Need to make Jupiter Brick
    {
    }

    private BiomeGenBase[] biomesForGeneration = { SCUranusBiomeGenBase.uranusFlat };
    ...
    public SCUranusChunkProvider(World par1World, long par2, boolean par4)
    {
    ...
    public void generateTerrain(int chunkX, int chunkZ, short[] idArray, byte[] metaArray)
    {
        ...
        for (int x = 0; x < SCUranusChunkProvider.CHUNK_SIZE_X; x++)
        {
            for (int z = 0; z < SCUranusChunkProvider.CHUNK_SIZE_Z; z++)
            {
                ...
                for (int y = 0; y < SCUranusChunkProvider.CHUNK_SIZE_Y; y++)
                {
                    if (y < SCUranusChunkProvider.MID_HEIGHT + yDev)
                    {
    ...
    {
        return SCUranusConfigManager.generateOtherMods ? "RandomLevelSource" : "VenusLevelSource";
    }
    ...
    public Chunk provideChunk(int par1, int par2)
    {
        ...
        final Chunk var4 = new Chunk(this.worldObj, ids, meta, par1, par2);
         ...
         SCUranusConfigManager.generateOtherMods)
         {
