...
{
    final short topBlockID = (short) StarcraftGasPlanets.UranusNitrogen.blockID;
    final byte topBlockMeta = 5;
    final short fillBlockID = (short) StarcraftGasPlanets.UranusHelium.blockID;
    final byte fillBlockMeta = 3;
    final short lowerBlockID = (short) StarcraftGasPlanets.UranusHydrogen.blockID;
    ...
    private final GCCoreMapGenDungeon dungeonGenerator = new GCCoreMapGenDungeon(StarcraftGasPlanets.UranusHydrogen.blockID, 14, 8, 16, 3);
    //Need to make Jupiter Brick
    {
    }

    private BiomeGenBase[] biomesForGeneration = { GCUranusBiomeGenBase.venusFlat };
    ...
    public GCUranusChunkProvider(World par1World, long par2, boolean par4)
    {
    ...
    public void generateTerrain(int chunkX, int chunkZ, short[] idArray, byte[] metaArray)
    {
        ...
        for (int x = 0; x < GCUranusChunkProvider.CHUNK_SIZE_X; x++)
        {
            for (int z = 0; z < GCUranusChunkProvider.CHUNK_SIZE_Z; z++)
            {
                ...
                for (int y = 0; y < GCUranusChunkProvider.CHUNK_SIZE_Y; y++)
                {
                    if (y < GCUranusChunkProvider.MID_HEIGHT + yDev)
                    {
    ...
    public Chunk provideChunk(int par1, int par2)
    {
        ...
        final Chunk var4 = new Chunk(this.worldObj, ids, meta, par1, par2);
         ...
         GCUranusConfigManager.generateOtherMods)
         {
    ...
    {
        return GCUranusConfigManager.generateOtherMods ? "RandomLevelSource" : "VenusLevelSource";
    }
