...
import mattparks.mods.starcraft.neptune.SCNeptuneConfigManager;
...
public class SCNeptuneChunkProvider extends ChunkProviderGenerate
{
    final short topBlockID = (short) JupiterBlocks.NeptuneNitrogen.blockID;
    final byte topBlockMeta = 5;
    final short fillBlockID = (short) JupiterBlocks.NeptuneHelium.blockID;
    final byte fillBlockMeta = 3;
    final short lowerBlockID = (short) JupiterBlocks.NeptuneHydrogen.blockID;
    ...
    public SCNeptuneBiomeDecorator biomedecoratorplanet = new SCNeptuneBiomeDecorator(SCNeptuneBiomeGenBase.neptuneFlat);

    private final World worldObj;
    private final GCCoreMapGenDungeon dungeonGenerator = new GCCoreMapGenDungeon(JupiterBlocks.JupiterHydrogen.blockID, 14, 8, 16, 3);
    //Need to make Jupiter Brick
    {
    }

    private BiomeGenBase[] biomesForGeneration = { SCNeptuneBiomeGenBase.neptuneFlat };
    ...
    public SCNeptuneChunkProvider(World par1World, long par2, boolean par4)
    {
    ...
    public void generateTerrain(int chunkX, int chunkZ, short[] idArray, byte[] metaArray)
    {
        ...
        for (int x = 0; x < SCNeptuneChunkProvider.CHUNK_SIZE_X; x++)
        {
            for (int z = 0; z < SCNeptuneChunkProvider.CHUNK_SIZE_Z; z++)
            {
                ...
                for (int y = 0; y < SCNeptuneChunkProvider.CHUNK_SIZE_Y; y++)
                {
                    if (y < SCNeptuneChunkProvider.MID_HEIGHT + yDev)
                    {
    ...
    {
        return SCNeptuneConfigManager.generateOtherMods ? "RandomLevelSource" : "VenusLevelSource";
    }
    ...
    public Chunk provideChunk(int par1, int par2)
    {
        ...
        final Chunk var4 = new Chunk(this.worldObj, ids, meta, par1, par2);
         ...
         SCNeptuneConfigManager.generateOtherMods)
         {
