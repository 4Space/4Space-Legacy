...
public class GCMercuryRoomSpawner extends GCCoreDungeonRoom
{
    ...
    public GCMercuryRoomSpawner(GCCoreMapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir)
    {
    ...
    protected void handleTileEntities(Random rand)
    {
        for (final ChunkCoordinates spawnerCoords : this.spawners)
        {
            ...
            if (spawner != null)
            {
                spawner.getSpawnerLogic().setMobID(GCMercuryRoomSpawner.getMob(rand));
            }
        }
    }
