package mattparks.mods.starcraft.saturn.wgen;
...
import mattparks.mods.starcraft.saturn.SCSaturnConfigManager;
...
public class SCSaturnChunkProvider extends ChunkProviderGenerate
{
    final short topBlockID = (short) JupiterBlocks.SaturnNitrogen.blockID;
    final byte topBlockMeta = 5;
    final short fillBlockID = (short) JupiterBlocks.SaturnHelium.blockID;
    final byte fillBlockMeta = 3;
    final short lowerBlockID = (short) JupiterBlocks.SaturnHydrogen.blockID;
    ...
    public SCSaturnBiomeDecorator biomedecoratorplanet = new SCSaturnBiomeDecorator(SCSaturnBiomeGenBase.saturnFlat);

    private final World worldObj;
    private final GCCoreMapGenDungeon dungeonGenerator = new GCCoreMapGenDungeon(JupiterBlocks.SaturnHydrogen.blockID, 14, 8, 16, 3);
    //Need to make Jupiter Brick
    {
    }

    private BiomeGenBase[] biomesForGeneration = { SCSaturnBiomeGenBase.saturnFlat };
    ...
    public SCSaturnChunkProvider(World par1World, long par2, boolean par4)
    {
    ...
    public void generateTerrain(int chunkX, int chunkZ, short[] idArray, byte[] metaArray)
    {
        ...
        for (int x = 0; x < SCSaturnChunkProvider.CHUNK_SIZE_X; x++)
        {
            for (int z = 0; z < SCSaturnChunkProvider.CHUNK_SIZE_Z; z++)
            {
                ...
                for (int y = 0; y < SCSaturnChunkProvider.CHUNK_SIZE_Y; y++)
                {
                    if (y < SCSaturnChunkProvider.MID_HEIGHT + yDev)
                    {
    ...
    {
        return SCSaturnConfigManager.generateOtherMods ? "RandomLevelSource" : "VenusLevelSource";
    }
    ...
    public Chunk provideChunk(int par1, int par2)
    {
        ...
        final Chunk var4 = new Chunk(this.worldObj, ids, meta, par1, par2);
         ...
         SCSaturnConfigManager.generateOtherMods)
         {
