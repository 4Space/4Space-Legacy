...
public class SCMakemakeChunkProvider extends ChunkProviderGenerate
{
    final short topBlockID = (short) MakemakeBlocks.MakemakeGrass.blockID;
    final byte topBlockMeta = 5;
    final short fillBlockID = (short) MakemakeBlocks.MakemakeDirt.blockID;
    final byte fillBlockMeta = 3;
    final short lowerBlockID = (short) MakemakeBlocks.MakemakeStone.blockID;
    ...
    public SCMakemakeBiomeDecorator biomedecoratorplanet = new SCMakemakeBiomeDecorator(SCMakemakeBiomeGenBase.makemakeFlat);

    private final World worldObj;
    {
    }

    private BiomeGenBase[] biomesForGeneration = { SCMakemakeBiomeGenBase.makemakeFlat };
    ...
    public SCMakemakeChunkProvider(World par1World, long par2, boolean par4)
    {
    ...
    public void generateTerrain(int chunkX, int chunkZ, short[] idArray, byte[] metaArray)
    {
        ...
        for (int x = 0; x < SCMakemakeChunkProvider.CHUNK_SIZE_X; x++)
        {
            for (int z = 0; z < SCMakemakeChunkProvider.CHUNK_SIZE_Z; z++)
            {
                ...
                for (int y = 0; y < SCMakemakeChunkProvider.CHUNK_SIZE_Y; y++)
                {
                    if (y < SCMakemakeChunkProvider.MID_HEIGHT + yDev)
                    {
    ...
    {
        return SCMakemakeConfigManager.generateOtherMods ? "RandomLevelSource" : "VenusLevelSource";
    }
    ...
    public Chunk provideChunk(int par1, int par2)
    {
        ...
        final Chunk var4 = new Chunk(this.worldObj, ids, meta, par1, par2);
         ...
         SCMakemakeConfigManager.generateOtherMods)
         {
