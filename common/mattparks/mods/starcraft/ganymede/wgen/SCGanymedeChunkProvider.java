...
import mattparks.mods.starcraft.ganymede.blocks.GanymedeBlocks;
...
public class SCGanymedeChunkProvider extends ChunkProviderGenerate
{
    ...
    final short fillBlockID = (short) GanymedeBlocks.GanymedeSubStone.blockID;
    final byte fillBlockMeta = 0;
    final short lowerBlockID = (short) GanymedeBlocks.GanymedeStone.blockID;
    ...
    public SCGanymedeBiomeDecorator biomedecoratorplanet = new SCGanymedeBiomeDecorator(SCGanymedeBiomeGenBase.ganymedeFlat);

    private final World worldObj;
    {
    }

    private BiomeGenBase[] biomesForGeneration = { SCGanymedeBiomeGenBase.ganymedeFlat };
    ...
    public SCGanymedeChunkProvider(World par1World, long par2, boolean par4)
    {
    ...
    public void generateTerrain(int chunkX, int chunkZ, short[] idArray, byte[] metaArray)
    {
        ...
        for (int x = 0; x < SCGanymedeChunkProvider.CHUNK_SIZE_X; x++)
        {
            for (int z = 0; z < SCGanymedeChunkProvider.CHUNK_SIZE_Z; z++)
            {
                ...
                for (int y = 0; y < SCGanymedeChunkProvider.CHUNK_SIZE_Y; y++)
                {
                    if (y < SCGanymedeChunkProvider.MID_HEIGHT + yDev)
                    {
    ...
    {
        return SCGanymedeConfigManager.generateOtherMods ? "RandomLevelSource" : "VenusLevelSource";
    }
    ...
    public Chunk provideChunk(int par1, int par2)
    {
        ...
        final Chunk var4 = new Chunk(this.worldObj, ids, meta, par1, par2);
         ...
         SCGanymedeConfigManager.generateOtherMods)
         {
