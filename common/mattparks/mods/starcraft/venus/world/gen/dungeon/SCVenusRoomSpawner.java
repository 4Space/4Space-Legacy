package mattparks.mods.starcraft.venus.world.gen.dungeon;
...
public class SCVenusRoomSpawner extends GCCoreDungeonRoom
{
	int sizeX;
	int sizeY;
	int sizeZ;
	...
	public SCVenusRoomSpawner(GCCoreMapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir)
	{
	...
	{
		return new SCVenusRoomSpawner(dungeon, x, y, z, dir);
	}
	...
	protected void handleTileEntities(Random rand)
	{
		for (final ChunkCoordinates spawnerCoords : this.spawners)
		{
			if (this.worldObj.getBlockId(spawnerCoords.posX, spawnerCoords.posY, spawnerCoords.posZ) == Block.mobSpawner.blockID)
			{
				...
				if (spawner != null)
				{
					spawner.getSpawnerLogic().setMobID(SCVenusRoomSpawner.getMob(rand));
				}
			}
		}
	}
