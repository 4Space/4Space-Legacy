...
import mattparks.mods.starcraft.eris.wgen.dungeon.GCErisRoomBoss;
import mattparks.mods.starcraft.eris.wgen.dungeon.GCErisRoomChests;
import mattparks.mods.starcraft.eris.wgen.dungeon.GCErisRoomEmpty;
import mattparks.mods.starcraft.eris.wgen.dungeon.GCErisRoomSpawner;
import mattparks.mods.starcraft.eris.wgen.dungeon.GCErisRoomTreasure;
...
public class SCErisChunkProvider extends ChunkProviderGenerate
{
    ...
    final short fillBlockID = (short) ErisBlocks.ErisDirt.blockID;
    final byte fillBlockMeta = 3;
    final short lowerBlockID = (short) ErisBlocks.ErisStone.blockID;
    ...
    public SCErisBiomeDecorator biomedecoratorplanet = new SCErisBiomeDecorator(SCErisBiomeGenBase.erisFlat);

    private final World worldObj;
    private final GCCoreMapGenDungeon dungeonGenerator = new GCCoreMapGenDungeon(ErisBlocks.ErisBrick.blockID, 14, 8, 16, 3);
    ...
    private BiomeGenBase[] biomesForGeneration = { SCErisBiomeGenBase.erisFlat };

    private final GCCoreMapGenBaseMeta caveGenerator = new SCErisGenCaves();
    ...
    public SCErisChunkProvider(World par1World, long par2, boolean par4)
    {
    ...
    public void createCraters(int chunkX, int chunkZ, short[] chunkArray, byte[] metaArray)
    {
        for (int cx = chunkX - 2; cx <= chunkX + 2; cx++)
        {
            ...
            {
                for (int x = 0; x < SCErisChunkProvider.CHUNK_SIZE_X; x++)
                {
                    for (int z = 0; z < SCErisChunkProvider.CHUNK_SIZE_Z; z++)
                    {
                        if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getNoise(x * SCErisChunkProvider.CHUNK_SIZE_X + x, cz * SCErisChunkProvider.CHUNK_SIZE_Z + z) / SCErisChunkProvider.CRATER_PROB)
                        {
    ...
    public void generateTerrain(int chunkX, int chunkZ, short[] idArray, byte[] metaArray)
    {
        ...
        for (int x = 0; x < SCErisChunkProvider.CHUNK_SIZE_X; x++)
        {
            for (int z = 0; z < SCErisChunkProvider.CHUNK_SIZE_Z; z++)
            {
                ...
                for (int y = 0; y < SCErisChunkProvider.CHUNK_SIZE_Y; y++)
                {
                    if (y < SCErisChunkProvider.MID_HEIGHT + yDev)
                    {
    ...
    {
        return SCErisConfigManager.generateOtherMods ? "RandomLevelSource" : "ErisLevelSource";
    }
    ...
    public Chunk provideChunk(int par1, int par2)
    {
        ...
        final Chunk var4 = new Chunk(this.worldObj, ids, meta, par1, par2);
         ...
         SCErisConfigManager.generateOtherMods)
         {
